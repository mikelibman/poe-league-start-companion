...
fn tail_loop(app: AppHandle, path: String, running: Arc<AtomicBool>) {
    ...
    while running.load(Ordering::SeqCst) {
        ...
        for event in extract_zone_events(&mut leftover, &chunk, &locale) {
            let _ = app.emit(
                ZONE_ENTERED_EVENT,
                ZoneEnteredPayload {
                    zone: event.zone,
...
fn extract_zone_events(
    leftover: &mut String,
    chunk: &str,
    locale: &dyn LogLocale,
) -> Vec<super::log_locale::ZoneEnteredEvent> {
...
mod tests {
    ...
    fn parses_a_complete_line_delivered_in_one_chunk() {
        ...
        let events = extract_zone_events(&mut leftover, chunk, &EnglishLocale);
        assert_eq!(events.len(), 1);
        assert_eq!(events[0].zone, "Lioneye's Watch");
        assert_eq!(leftover, "");
    }
    ...
    fn buffers_a_line_split_across_two_chunks() {
        ...
        let events = extract_zone_events(&mut leftover, first_half, &EnglishLocale);
        assert!(events.is_empty());
        assert_eq!(leftover, first_half);

        let events = extract_zone_events(&mut leftover, second_half, &EnglishLocale);
        assert_eq!(events.len(), 1);
        assert_eq!(events[0].zone, "Lioneye's Watch");
        assert_eq!(leftover, "");
    }
    ...
    fn parses_multiple_lines_in_one_chunk_and_keeps_trailing_partial() {
        ...
        assert_eq!(events[0].zone, "Town");
        assert_eq!(events[1].zone, "The Coast");
